...
const AIAssistant = () => {
    ...
    const handleSendMessage = async () => {
        ...
        try {
            ...
            const aiText =
                result.text ||
                "Connection timing out. Please re-initiate sync.";
            ...
            setMessages((prev) => [
                ...prev,
                {
                    ...
                    content: isQuota
                        ? "AI capacity reached. Please continue via WhatsApp while we scale compute."
                        : "Protocol error. Please sync via WhatsApp or email scrollfuel@gmail.com.",
                },
            ]);
        } finally {
            setIsLoading(false);
    ...
    return (
        <div className="fixed bottom-4 right-4 z-50 font-sans">
            ...
            <button
                onClick={openWhatsApp}
                className="absolute bottom-0 right-0 w-12 h-12 bg-primary text-white rounded-full flex items-center justify-center shadow-lg active:scale-90"
            >
                <MessageCircle size={20} fill="currentColor" />
            </button>
            ...
            {/* Chat Window */}
            <div
                className={`absolute bottom-16 right-0 w-[95vw] sm:w-96 h-[80vh]
                rounded-3xl flex flex-col transition-all duration-300 origin-bottom-right overflow-hidden
            ...
            >
                ...
                >
                    {messages.map((m, i) => (
                        ...
                        >
                            <div
                                className={`max-w-[85%] p-3 rounded-xl text-xs ${m.role === "user"
                ...
                <div className="px-4 py-2 flex gap-2 overflow-x-auto">
                    <button
                        onClick={() =>
                            setInput("Where is your Nagpur studio?")
                        }
                    ...
                    <button
                        onClick={() =>
                            setInput("How to start a project?")
                        }
                        className="px-3 py-1 text-xs border rounded-full"
                    >
                        Workflow
                ...
                <div className="p-4">
                    ...
                    >
                        <input
                            ...
                            className="w-full border rounded-xl px-4 py-3 pr-12 focus:outline-none"
                        />

                        <button
                            type="submit"
                            disabled={isLoading || !input.trim()}
                            className="absolute right-2 top-1/2 -translate-y-1/2 p-3 text-primary active:scale-90 disabled:opacity-30"
                        >
                            <Send size={18} />
                        </button>
                    </form>
